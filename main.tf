locals {
  ...
  mapped_vars = {
    for k, v in var.variables_set : k.name => v if length(k.name) != 0
  }
}
...
resource "azuredevops_serviceendpoint_azurerm" "this" {
  for_each = var.create_service_endpoints ? nonsensitive({
    for k in var.service_endpoint_args : index(var.service_endpoint_args, k) => k
  }) : {}
...
resource "azuredevops_variable_group" "this" {
  count = length(local.mapped_vars) != 0 ? 1 : 0
